...
public class Tagger{
	...
	public String preProccessText(String text){
            ...
            //Adiciona espaÃ§o para acronimos no comeÃ§o/final frase
		text = espacaTexto(text);
		//Expande as datas	
		text = expandirData(text);
		//Retira erros recorrentes nos sumÃ¡rios
		text = retiraErrosRecorrentes(text);
		//Retira pontuaÃ§Ã£o e caracteres especiais
...
// retirado porque estava tirando a "base" do trecho "núcleos da base"		cogroo.nameFinder(sc);
		...
		cogroo.tagger(sc);
		tokens = sc.getTokens();
                //flair - está como PRP_ e devia ser ADJ_F_S
                //peritoneal - está como PRP_ e devia ser ADJ_F_S
	...
	//Tagger para Interface grÃ¡fica
...
// retirado porque estava tirando a "base" do trecho "núcleos da base"		cogroo.nameFinder(selecionado);
		...
		for (String sentenca : sentencas) {
			...
			//Procura onde estÃ£o os termos seleciontexto_expandido = text_selecionado_separado.get(k).getLexeme();ados
			//Compara um termo com o primeiro do vetor separado, caso encontre, ve se os termos
			//seguintes tambÃ©m sÃ£o os esperados
			boolean igual = false;
			...
			for(int i=0; i < tokens.size(); i++){
				...
				if(igual){
					for(int k=0; k < text_selecionado_separado.size(); k++){
						texto_expandido += text_selecionado_separado.get(k).getLexeme() + " "; //Para deixar o texto da regra expandido quando guardar no bd, ao invés do selecionado.
...
// retirado porque estava tirando a "base" do trecho "núcleos da base"		cogroo.nameFinder(selecionado);
		...
		for (String sentenca : sentencas) {
			...
			//Procura onde estÃ£o os termos selecionados
			//Compara um termo com o primeiro do vetor separado, caso encontre, ve se os termos
			//seguintes tambÃ©m sÃ£o os esperados
			boolean igual = false;
...
//		return text.replaceAll("[-!?><=%;/#,@*]", " ");
	...
	private String expandirData(String text){
		 ...
		 if (Pattern.compile("[\\d]+[\\d]+[/]+[\\d]+[\\d]+[/]+[\\d]+[\\d]").matcher(text).find()
	    	...
	    	text = text.replace("/mar/"," de marÃ§o de ");
	    	...
	    	text = text.replace("/03/"," de marÃ§o de ");
	    ...
	    if (Pattern.compile("[\\d]+[\\d]+[/]+[\\d]+[\\d]").matcher(text).find()
	    	...
	    	text = text.replace("/mar"," de marÃ§o");
	    	...
	    	text = text.replace("/03"," de marÃ§o");
