...
public class Tagger{
	...
	public List <String> frasesNegativas;
        ...
        private int contadorPosiÁıesPassadas = 0; //Para manter registro de quantos tokens j· passaram nas frases anteriores, j· que ele sÛ manda frase por frase e o i pra localizar a posiÁ„o comeÁa de novo.
                                                  //Por isso deu problema e ele localizava sÛ na primeira frase.
	...
	public String preProccessText(String text){
		...
		//Retira Stopwords - mÈtodo retirado para identificar melhor a diferenÁa entre
                //achado radiolÛgico e termo morfolÛgico, pois os termos morfolÛgicos em algumas regras, s„o precedidos por artigos
...
// retirado porque estava tirando a "base" do trecho "n˙cleos da base"		cogroo.nameFinder(selecionado);
		...
		for (String sentenca : sentencas) {
			...
			//Procura onde est√£o os termos seleciontexto_expandido = text_selecionado_separado.get(k).getLexeme();ados
			...
			for(int i=0; i < tokens.size(); i++){
				...
				if(igual){
					for(int k=0; k < text_selecionado_separado.size(); k++){
						texto_expandido += text_selecionado_separado.get(k).getLexeme() + " "; //Para deixar o texto da regra expandido quando guardar no bd, ao invÈs do selecionado.
...
//O teste abaixo foi colocado para identificar termos de negaÁ„o, por enquando somente com "sem"
...
//							if(testeSubregra){ trocado pela linha de baixo para manter os loops de regra e subregra parecidos
								...
								t.setTrechoEncontrado(trecho);
                                                                ...
                                                                //PosiÁ„o dos termos
                                                                t.setPosInicial(i+contadorPosiÁıesPassadas);
                                                                t.setPosFinal(i+contadorPosiÁıesPassadas+r.getNumTermos()-1);
                        ...
                        //Incrementa contador das posiÁıes das frases passadas
                        contadorPosiÁıesPassadas += tokens.size();
		}
                
                //Reinicia o contador para o prÛximo texto
                contadorPosiÁıesPassadas = 0;
		return encontrados;	
	}
